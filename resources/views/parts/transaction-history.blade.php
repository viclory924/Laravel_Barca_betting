<div class="row played">
	<div class="col-xs-12 field-left padding-none">
		<div class="inner-addon right-addon mobile-no-margin">
			<div style="text-align:left" class="jq_trans_head">
				@if(isset($date_from))
				...
				@endif
				<input type="text" id="datepicker" value="@php echo date('Y-m-d') @endphp" class="history_datepicker" placeholder="{{ __('Месяц')}}"> <span class="jq_new_transactions">{{ __('Найти')}}</span>
			</div>
		</div>
	</div>
</div>
	...
	<div class="col-xs-11 col-sm-12 text-left padding-none jq_hisory_trans_table">
		<div id="data-div">
			<table>
				...
				<tbody id="data-table" class="jq_trans_table_body">
					@if(isset($transactions) && $transactions != '')
						@if(is_array($transactions->transactions->transaction))
							...
							@endforeach
						@else
							@if($transactions->transactions->transaction->type == 'DEPOSIT')
			...
			<div style="margin-top:2px" class="jq_trans_pagin">
				@if(isset($transactions) && $transactions != '')
