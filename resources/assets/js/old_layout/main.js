...
(function ($) {
   ...
   /* var runningAnimationFrame = true;
    ...
    $(document).on('click', '.games-filter .js-filter-games', function (e) {
        ...
        // loader(true);
        if ($(this).parent('.bigtab').hasClass('active-filter')) {
            // loader(false);
            return;
        }
        ...
        //if (!$(this).hasClass('active')) {
        $('.games-filter .js-filter-games').parent('.bigtab').removeClass('active-filter');
        $(this).parent('.bigtab').addClass('active-filter');
        //} else {
        ...
        if (tag == 'last') {
			...
			if( !logged )
			{
				...
				$('.js-filter-games').parent('.bigtab').removeClass('active-filter');
				$(this).parent('.bigtab').addClass('active-filter');
    ...
    function placeGames(games) {
        ...
        if (games.result.length > 0) {
            $.each(games.result,function(index,value){
			    var game_item = '<div class="game-link">';

			    if (filter_params.type == 'new' || value.is_new > 0) {
                    game_item += '<div class="badge-wrapper"> <i class="badge-new" title="New"></i><span class="badge-label false">New</span></div>';
                }
                ...
                var jackpot = '';
                if (value.jackpot > 0) {
                    jackpot = '<div class="show jackpots" data-reactid=".0.$8270-mega-fortune-catfeaturedGames.0.0"><p data-reactid=".0.$8270-mega-fortune-catfeaturedGames.0.0.0">£2,306,664.00</p></div>';
                }
                ...
                game_item += '<a href="#">'+
                    ...
                    '</span>'+
                    jackpot +
                    '<span class="play-game-title">' + overlay_text + '</span>'+
                '</a>';
                ...
                if (filter_params.sub_container == true) {
                    ...
                    $(game_item).insertAfter($('.games-list header.games-section'));
                }
            });
        } else {
		 $('<p class="not-found-text">NO GAMES FOUND</p>').insertAfter($('.games-list.games-items header'));
        }
    ...
    $('.js-further-step').on('click', function(e){
        ...
        if (dataStep == 'quick-register-form') {
            ...
            if ($('form#full-registration-step1').validate().form() && $('form#full-registration-step2').validate().form()) {
                ...
                $.post(
                    $('form#quick-register-form').attr('action'),
                    registerObj,
                    function(result){
