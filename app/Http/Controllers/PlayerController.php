...
//		    }
    ...
    {
	    $player = Player::where('username', $request->username)
...
//                );
	...
	public function makeWithdraw(Request $request)
	{
		...
		return json_encode(array('status' => 1, 'result' => $url));
...
//			\Session::flash('sucess_message','Doc Successfully Upload');
            return json_encode(array('status' => 1, 'message' => 'file.sucessfully.uploaded'));
        }else{
		    return json_encode(array('status' => 0, 'message' => 'invalid.file.type'));
        }
...
//        }
    ...
    public function getWithdrawals() {
	    if (!\Auth::user()) {
	        ...
	        $res = \App\StaygamingBO::getWithdrawTransactions(\Auth::user()->player_id);
        }
        return $res;
    }
    ...
    public function cancelPendingWithdraw(Request $request) {
        if (!\Auth::user()) {
            ...
            $res = \App\StaygamingBO::cancelPendingWithdrawTransactions($request);
        }
    ...
    public function addBankAccount(Request $request) {
        if (!\Auth::user()) {
            ...
            $res = \App\StaygamingBO::addBankAccount($request, \Auth::user()->player_id);
        }
        return $res;
    }
    ...
    public function getBankAccount(Request $request) {
        if (!\Auth::user()) {
            ...
            $res = \App\StaygamingBO::getBankAccount(\Auth::user()->player_id);
        }
        return $res;
    }
    ...
    public function getBonuses(Request $request) {
        if (!\Auth::user()) {
            ...
            $res = \App\StaygamingBO::getBonuses(\Auth::user()->player_id);
        }
        return $res;
    }
}
